...
class TensorboardSummaryHook:
    ...
    def __init__(
        ...
        artifacts_dir,
        comparison_pairs=[],
        volumetric_views=False,
        video_dims="CTHW",
        image_preds_classes_spec={"class": None, "pred": None, "image": None,},
    ):
        """
        ...
        :param volumetric_views: for use if 3D data is a 3D image, this saves all 3 2D projections, e.g. axial,
          sagittal, coronal views of a brain.
        :type volumetric_views: bool
        :param video_dims: if 3D data is a video clip, supports "CTHW" or "TCHW" dimension orderings.
        :type video_dims: str
        ...
        self.show_all_axes = volumetric_views
        supported_video_dims = ["CTHW", "TCHW"]
        if video_dims not in supported_video_dims:
            raise ValueError(
                "%s is not in supported video dims: %s" % supported_video_dims
            )
    ...
    def end_epoch(self, message):
        ...
        for inp, out in self.comparison_pairs:
            if message["inputs"][inp].ndim == 1 and message["outputs"][out].ndim == 1:
                # if outputs are valid probabilities and classification is binary, write pr curve
                if (
                    np.max(message["inputs"][inp]) <= 1
                    and np.max(message["outputs"][out]) <= 1
                    and not np.all(
                        message["outputs"][out]
                        == message["outputs"][out].astype(np.int)
                    )
                ):
                    self.write_pr_curve(
                    ...
                    # write negative class pr curve
                    self.write_pr_curve(
                        "{}_Vs_{}_negative_class/pr_curve".format(inp, out),
                        1 - message["inputs"][inp],
                        1 - message["outputs"][out],
                        epoch,
                    )

                if np.all(
                    message["outputs"][out] == message["outputs"][out].astype(np.int)
                ):
    ...
    def write_volumetric_image(self, name, value, global_step):
        ...
        if self.video_dims == "CTHW":
            v = np.transpose(value, [0, 2, 1, 3, 4])
        elif self.video_dims == "TCHW":
            v = value

        if v.shape[2] != 3 and v.shape[2] != 1:
            # convert non-RGB multi-channel to greyscale
        ...
        self.writer.add_video(
            name + "_axis_1" * self.show_all_axes,
            torch_value,
            fps=10,
    ...
    def write_vector(self, name, value, global_step):
        if not np.all(np.isinf(value)) and not np.all(np.isnan(value)):
